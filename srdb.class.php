...
//                }
    ...
    public function db_escape($string)
    {
        if ($this->use_pdo() && method_exists($this->db, 'quote')) {
            ...
            return mysqli_real_escape_string($this->db, stripslashes($string));
        }
    }
    ...
    public function recursive_unserialize_replace($from = '', $to = '', $data = '', $serialised = false)
    {
        ...
        try {
            ...
            $base64_serialized = false;

            if (is_bool($data) || is_null($data) || is_integer($data) || is_float($data)) {
                if ($serialised) {
            ...
            if (is_object($data)) {
                ...
                if (function_exists('json_decode')) {
                    ...
                    if ($retval != null && !is_bool($retval) && is_array($retval)) { // If decoded successfully
                        $base64_serialized = true;
                        $data = $this->recursive_unserialize_replace($from, $to, $retval);
                    }

                    if ($base64_serialized === false) {
                ...
                if (!$json_serialized || !$base64_serialized) {
                ...
                throw new Exception("Undefined data type: ".gettype($data));
            }


            if ($serialised) {
                return serialize($data);
            } elseif ($base64_serialized) {
